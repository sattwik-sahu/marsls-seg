...
class ChannelMaskingViTInput(TensorClass):
    image: torch.Tensor
    mask: Optional[torch.Tensor] = None


class ChannelMaskingViT(torch.nn.Module):
    """
    ...
    def forward(self, x: ChannelMaskingViTInput | torch.Tensor) -> torch.Tensor:
        ...
        if apply_mask:
            patch_tokens = patch_tokens[:, x.mask]  # type: ignore
