    ...
    def train_epoch(
        ...
        for batch in dataloader:
            ids_keep,ids_drop=generate_uniform_mask(
                ...
                n_channels=self._model.context_encoder.n_channels,
                device=self.device)
        ...
        return {"loss": output.loss.total.item()}
    
    def _generate_svd_chart(self, target_encodings: torch.Tensor) -> go.Figure | None:
        """SVD Health Check for the 1792 tokens."""
        flat_features = target_encodings.reshape(-1, target_encodings.shape[-1]).detach().float()
        flat_features = flat_features - flat_features.mean(dim=0, keepdim=True)
        try:
            singular_values = torch.linalg.svdvals(flat_features)
            normalized_sv = (singular_values / (singular_values[0] + 1e-8)).cpu().numpy()
        except RuntimeError:
            return None

        fig = go.Figure()
        fig.add_trace(go.Scatter(y=normalized_sv, mode="lines+markers", name="SVD Decay"))
        fig.update_layout(title="Spatio-Spectral Latent Health", template="plotly_white")
        return fig
    ...
    def evaluate(
        ...
        for batch in dataloader:
            ...
            with torch.no_grad():
                ...
                flat_target = jepa_output.target_encoding.reshape(-1, jepa_output.target_encoding.shape[-1])
                ...
                flat_pred = jepa_output.prediction.reshape(-1, jepa_output.prediction.shape[-1])
        ...
        metrics_payload: dict[str, Any] = {
            ...
            "val/diagnostics/prediction_cosine_similarity": sum(cos_sims) / len(cos_sims),
            "val/diagnostics/latent_dimension_std": sum(latent_stds) / len(latent_stds),
            "val/diagnostics/latent_pred_std": sum(latent_pred_stds) / len(latent_pred_stds),
        }
